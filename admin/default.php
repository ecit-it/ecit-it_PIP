<html><head>
<meta http-equiv="Content-Type" content="text/html; charset=gb2312">
<title>大学生实习平台--后台</title>
<link rel="stylesheet" type="text/css" href="css/font.css">
</head>
<body topmargin="0" leftmargin="0" bottommargin="0" class="scrollbar">
<table width="1003" align="center" cellpadding="1" cellspacing="0" bordercolor="#CCCCCC" bgcolor="#999999">
  <tr>
    <td><table width="1003" border="0" align="center" cellpadding="0" cellspacing="0">
      <tr>
        <td height="90" bgcolor="#FFFFFF"><div align="center">
            <table width="1003" height="20">
  <tr>
    <td width="880" height="20" align="center" bgcolor="#FFFFFF">大学生实习工作平台-后台管理系统欢迎管理员您的光临！请及时处理申请单！并合理安排你的工作时间。</td><td width="111" bgcolor="#FFFFFF"><a href="../index.php">网站前台</a><a href="../admin/logout.php"><font color="#FF0000">&nbsp;&nbsp;&nbsp;登出</font></a></td></tr>
  </table><IFRAME frameBorder=0 id=top name=top scrolling=no src="top.php" 
     style="HEIGHT: 90px; VISIBILITY: inherit; WIDTH: 1003px; Z-INDEX: 3"> </IFRAME>
        </div></td>
      </tr>
    </table>
      <table width="1003" height="620" border="0" align="center" cellpadding="0" cellspacing="0">
        <tr>
          <td width="212" height="220" valign="top" bgcolor="#FFCF60" id="lt" style="display:"><div align="center">
              <IFRAME frameBorder=0 id=left name=left src="left.php" 
      style="HEIGHT: 100%; VISIBILITY: inherit; WIDTH: 212px; Z-INDEX: 2"> </IFRAME>
          </div></td>
          <td width="13" height="584" background="images/bg_line.gif"><div align="center"></div></td>
          <td width="778" bgcolor="#FFFFFF" id="mn"><div align="center">
              <IFRAME frameBorder=0 id=main name=main scrolling=yes src="lookdd.php" 
      style="HEIGHT: 100%; VISIBILITY: inherit; WIDTH: 778px; Z-INDEX: 1"> </IFRAME>
          </div></td>
        </tr>
      </table></td>
  </tr>
</table>
</body>
</html>
